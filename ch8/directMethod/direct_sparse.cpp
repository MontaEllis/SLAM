...
/***************************
 * this section shows the sparse directly method in RGBD
 * *************************/

// world coordinate 3D point + grayscale value
struct Measurement
{
...
int main(int argc, char** argv)
{
  ...
  //we set the first image as reference, use direct method to the following images
  for(int index=0;index<10;index++)
  {
    ...
    if(index ==0)
    {
      // extract FAST feature point in the first frame
      vector<cv::KeyPoint> keypoints;
      cv::Ptr<cv::FastFeatureDetector> detector = cv::FastFeatureDetector::create();
      detector->detect(color, keypoints);
      for(auto kp:keypoints)
      {
	// get rid of the points which are close to the EdgeSE3ProjectDirect
    ...
    // use direct method to compute the movement of camera
    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    poseEstimationDirect(measurements, &gray,K,Tcw);
    chrono::steady_clock::time_point t2 = chrono::steady_clock::now();
    chrono::duration<double> time_used = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
    cout << "direcrt method costs time : " << time_used.count() << "seconds." <<endl;
